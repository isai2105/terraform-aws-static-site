# Inputs to the prod environment root.
#
# There is exactly one rule deciding what belongs here, and it is what keeps
# `envs/stage` and `envs/prod` from drifting into two unrelated configurations:
# **a value is a variable here only if it differs between accounts, never if it
...
# and they are identical across every environment in a given clone. That is why this
# file is the same nine variables stage declares, with the same rules: a prod
# root that took an input stage does not, or validated one of these more
# loosely, would be a prod that stage cannot vouch for.
#
...
# reason the bootstrap gives about its own `Env` tag: `envs/prod` has exactly
# one correct value for it, and exposing it as an input would let
# `-var environment=stage` create stage-named resources under prod's state key,
...
variable "name_prefix" {
  description = <<-EOT
    ...
    It is deliberately not per-environment. The environment name is already in
    the composed bucket name, so stage and prod get distinct buckets from one
    prefix, and a prefix that differed between them would put prod's bucket
    outside the single pattern the apply role grants.
  EOT
  type        = string

  # The module validates this value too, and the repetition is deliberate rather
  # than redundant: this check fails naming `envs/prod/terraform.tfvars`, which
