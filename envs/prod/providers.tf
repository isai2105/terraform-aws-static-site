...
locals {
  ...
  tags = {
    ...
    # `environment` input, and this is the directory that precondition was
    # written for: a prod root copied from a stage root keeps the tag it was
    # copied with, so an `Env = "stage"` here would survive review, apply
    # cleanly, and leave prod's resources answering to stage's teardown query
    # and to neither environment's own. A shared reference cannot make that
    # mistake; a second literal is how it gets made.
